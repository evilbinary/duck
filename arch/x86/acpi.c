...
void lapic_timer_init() {
  ...
  // TSC-Deadline mode,vector 0x20, program target value in IA32_TSC_DEADLINE
  // MSR.
...
int lapic_init() {
  ...
  val |= 0x100;  // Spurious Interrupt Vector Register bit 8 to start receiving
                 // interrupts
...
void lapic_send_init(u32 id) {
  ...
  mem_write32(APIC_BASE + LAPIC_ICRLO, ICR_INIT | ICR_PHYSICAL | ICR_ASSERT |
...
void lapic_send_start(u32 id, u32 vec) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    mem_write32(APIC_BASE + LAPIC_ICRLO, vec | ICR_STARTUP | ICR_PHYSICAL |
                                             ICR_ASSERT | ICR_EDGE |
                                             ICR_NO_SHORTHAND);
...
void lapic_eoi(void) { mem_write32(APIC_BASE + LAPIC_EOI, 0); }
