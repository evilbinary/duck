...
void timer_init(int hz) {
  ...
  // io_write32(TIMER0_BASE + TIMER_CONTROL, 0);
  // io_write32(TIMER0_BASE + TIMER_BGLOAD, 0);
  // io_write32(TIMER0_BASE + TIMER_LOAD, hz);

  // // u32 val=io_read32(TIMER0_BASE + TIMER_CONTROL);
  // // val|=0xe2;
  // io_write32(TIMER0_BASE + TIMER_CONTROL, 0xe2);

  io_write32(TIMER0_BASE + TIMER_LOAD, hz);

  u32 reg = TIMER_CTRL_PERIODIC | TIMER_CTRL_EN | TIMER_CTRL_32BIT |
            TIMER_CTRL_INTREN | TIMER_CTRL_32BIT | TIMER_CTRL_DIV1;
  io_write8(TIMER0_BASE + TIMER_CONTROL, reg);
