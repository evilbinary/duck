/*******************************************************************
 * Copyright 2021-present evilbinary
 * 作者: evilbinary on 01/01/20
 * 邮箱: rootdebug@163.com
 ********************************************************************/
#include "sdhci.h"
#include "sunxi-sdhci.h"
#include "gpio.h"
#include "gpio/sunxi-gpio.h"
#include "t113-ccu.h"
#include "mmc_cache.h"

#define CACHE_COUNT 1
#define SECTOR_SIZE (512 * CACHE_COUNT)

void sdhci_dev_init(sdhci_device_t *sdhci_dev) {
  sdhci_sunxi_pdata_t *pdat =
      kmalloc(sizeof(sdhci_sunxi_pdata_t), KERNEL_TYPE);
  kmemset(pdat, 0, sizeof(sdhci_sunxi_pdata_t));
  sdhci_dev->data = pdat;
  pdat->high_capacity = 0;
  pdat->read_bl_len = BYTE_PER_SECTOR;
  pdat->rca = 0;
  pdat->isspi = 0;
  pdat->write_bl_len = BYTE_PER_SECTOR;
  pdat->virt = MMC_BASE;
  pdat->port = 0;

  pdat->voltage = MMC_VDD_27_36;
  pdat->clock = 52000000;  // 工作时钟上限；识别阶段用 400kHz，见 sdhci_sunxi_probe
  pdat->width = MMC_BUS_WIDTH_4;

  pdat->reset = 8;
  pdat->clk = 162;
  pdat->clkcfg = 2;
  pdat->cmd = 2;
  pdat->cmdcfg = 2;
  pdat->dat0 = 161;
  pdat->dat0cfg = 2;
  pdat->dat1 = 160;
  pdat->dat1cfg = 2;
  pdat->dat2 = 165;
  pdat->dat2cfg = 2;
  pdat->dat3 = 164;
  pdat->dat3cfg = 2;
  pdat->dat4 = -1;
  pdat->dat4cfg = -1;
  pdat->dat5 = -1;
  pdat->dat5cfg = -1;
  pdat->dat6 = -1;
  pdat->dat6cfg = -1;
  pdat->dat7 = -1;
  pdat->dat7cfg = -1;
  pdat->cd = 166;
  pdat->cdcfg = 0;

  // sd mmc0
  page_map(MMC_BASE, MMC_BASE, 0);

  sdhci_sunxi_probe(sdhci_dev);

  if (mmc_cache_init(sdhci_dev) < 0) {
    log_error("t113 mmc cache init failed, running uncached\n");
  }

  kprintf("sdh dev init end\n");
}

static int t113_hw_read(void *ctx, u32 lba, u32 nsec, void *buf) {
  u32 r = mmc_read_blocks((sdhci_device_t *)ctx, lba, nsec, (u8 *)buf);
  return r == 0 ? -1 : 0;
}

int sdhci_dev_port_read(sdhci_device_t *sdhci_dev, char *buf, u32 len) {
  u32 bno = sdhci_dev->offsetl / BYTE_PER_SECTOR;
  u32 boffset = sdhci_dev->offsetl % BYTE_PER_SECTOR;

  if (mmc_cache_read(sdhci_dev, bno, boffset, len, buf, t113_hw_read,
                     sdhci_dev) >= 0) {
    return (int)len;
  }

  {
    u32 ret = 0;
    u32 bcount = (len + boffset + BYTE_PER_SECTOR - 1) / BYTE_PER_SECTOR;
    u32 bsize = bcount * BYTE_PER_SECTOR;

    if (bsize > sdhci_dev->read_buf_size) {
      if (sdhci_dev->read_buf) kfree(sdhci_dev->read_buf);
      sdhci_dev->read_buf = kmalloc(bsize, DEVICE_TYPE);
      sdhci_dev->read_buf_size = bsize;
    }

    ret = mmc_read_blocks(sdhci_dev, bno, bcount, sdhci_dev->read_buf);
    if (ret == 0) {
      return -1;
    }
    kmemmove(buf, sdhci_dev->read_buf + boffset, len);
    return (int)len;
  }
}

int sdhci_dev_port_write(sdhci_device_t *sdhci_dev, char *buf, u32 len) {
  u32 ret = 0;
  u32 bno = sdhci_dev->offsetl / BYTE_PER_SECTOR;
  u32 boffset = sdhci_dev->offsetl % BYTE_PER_SECTOR;
  u32 bcount = (len + boffset + BYTE_PER_SECTOR - 1) / BYTE_PER_SECTOR;
  u32 bsize = bcount * BYTE_PER_SECTOR;

  if (bsize > sdhci_dev->write_buf_size) {
    if (sdhci_dev->write_buf) kfree(sdhci_dev->write_buf);
    sdhci_dev->write_buf = kmalloc(bsize, DEVICE_TYPE);
    sdhci_dev->write_buf_size = bsize;
  }

  ret = mmc_write_blocks(sdhci_dev, buf, bno, bcount);
  mmc_cache_after_write(sdhci_dev, bno, bcount, NULL);

  return ret;
}
