 ...
 ********************************************************************/
 ...
 static int sunxi_transfer_command(sdhci_sunxi_pdata_t *pdat, sdhci_cmd_t *cmd,
   ...
   /* 真机 400kHz 下 CMD2(R2) 等长响应需要远长于 10000 次空转 */
   timeout = 0x100000;
   do {
   ...
   /*
    * 数据命令的 DATA_OVER 可能与 COMMAND_DONE 同时到达。这里只清命令完成，
    * 留下数据状态给 read_bytes/write_bytes；无数据命令则清全部状态。
    */
   io_write32(pdat->virt + SD_RISR, dat ? SDXC_COMMAND_DONE : 0xffffffff);
   return TRUE;
 }
 ...
 static int read_bytes(sdhci_sunxi_pdata_t *pdat, u32 *buf, u32 blkcount,
   ...
   while ((!err) && count >= sizeof(u32) && timeout--) {
   ...
   } while (!done && !err && timeout--);
 
   if (err || count || !done) {
     kprintf("mmc read failed risr=%x star=%x left=%d\n", status,
             io_read32(pdat->virt + SD_STAR), (u32)count);
     io_write32(pdat->virt + SD_GCTL,
 ...
 static int sdhci_sunxi_reset(sdhci_device_t *sdhci) {
   ...
   io_write32(pdat->virt + SD_GCTL, SDXC_HARDWARE_RESET);
 #ifdef CCU_SMHC0_CLK_REG
   /* T113/D1 等 NCAT2：卡侧硬件复位 */
   io_write32(pdat->virt + SD_HWRST, 0);
   for (volatile int i = 0; i < 1000; i++)
     ;
   io_write32(pdat->virt + SD_HWRST, 1);
 ...
 #ifdef CCU_SMHC0_CLK_REG
   /* T113/D1：PLL_PERI0 布局与 A20 不同，用平台已有计算 */
   extern uint32_t sunxi_clk_get_peri1x_rate(void);
   uint32_t rate = sunxi_clk_get_peri1x_rate();
   return rate ? rate : 600000000;
 #else
 ...
 // SD card controller clock（CCU 模时钟；与 U-Boot mmc_set_mod_clk 对齐）
 uint32_t clk_sdc_config(uint32_t reg, uint32_t freq) {
   log_debug("clk sdc freq %d\n", freq);
   uint32_t in_freq = 0;
   uint32_t reg_val = (1 << 31);
   if (freq <= 24000000) {
     reg_val |= (0 << 24);  // OSC24M / HOSC
     in_freq = 24000000;
     log_debug("clk OSC24M %d\n", in_freq);
   } else {
 #ifdef CCU_MMC_CTRL_PLL_PERIPH1X
     /* T113：源 1 = PLL_PERI(1x)，模时钟仍有隐藏 /2，用半频算分频 */
     reg_val |= CCU_MMC_CTRL_PLL_PERIPH1X;
     in_freq = pll_periph_get_freq() / 2;
 #else
   ...
   /* determine delays（老 sunxi 有 phase 位；T113 宏为 no-op） */
 ...
 #ifdef CCU_MMC_CTRL_N
   /* T113/D1：M[3:0] N[9:8] SRC[26:24] EN[31]，无 delay/prediv@16 */
   reg_val |= CCU_MMC_CTRL_N(prediv) | CCU_MMC_CTRL_M(div);
   reg_val |= CCU_MMC_CTRL_SCLK_DLY(samp_phase) | CCU_MMC_CTRL_OCLK_DLY(out_phase);
 #else
   /* A20/V3S/H3 等旧布局 */
 ...
 static int sdhci_sunxi_setclock(sdhci_device_t *sdhci, u32 clock) {
   ...
   /* 关卡时钟，清内部分频：频率只由 CCU 模时钟决定（勿再用 pdat->clock 二次分频） */
   rval = io_read32(pdat->virt + SD_CKCR);
   rval &= ~(SDXC_CARD_CLOCK_ON | 0xff);
   io_write32(pdat->virt + SD_CKCR, rval);
 ...
 #ifdef CCU_SMHC0_CLK_REG
   /*
    * T113 的 SMHC 使用新 timing 寄存器。软件采样延迟设为 0；
    * 不设置 SW_EN 时数据采样可能沿用未校准值，命令正常但读数据 CRC 失败。
    */
   io_write32(pdat->virt + SD_SAMP_DL, 1 << 7);
 #endif
 ...
 static int sd_send_op_cond(sdhci_device_t *hci) {
   ...
   do {
     ...
     if (pdat->isspi) {
       ...
       /* 传输失败或未就绪都继续；勿把失败当成成功跳出 */
       if (!sdhci_sunxi_transfer(hci, &cmd, NULL)) {
         log_warn("sd_send_op_cond acmd41 transfer fail\n");
         continue;
       }
   ...
   if (retries <= 0 || !(cmd.response[0] & OCR_BUSY)) {
