...
#if USE_DYNAMIC_MEMORY
  struct fat_fs_struct* fs = kmalloc(sizeof(struct fat_fs_struct),KERNEL_TYPE);
  if (!fs) {
...
#if USE_DYNAMIC_MEMORY
  struct fat_file_struct* fd = kmalloc(sizeof(struct fat_file_struct),KERNEL_TYPE);
  if (fd==NULL) return 0;
#else
...
#if USE_DYNAMIC_MEMORY
  struct fat_dir_struct* dd = kmalloc(sizeof(struct fat_dir_struct),KERNEL_TYPE);
  if (!dd) return 0;
#else
