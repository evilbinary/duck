...
static inline void t113_de_set_mode(t113_s3_lcd_t *pdat) {
  ...
  val = read32((u32)&clk->rst_cfg);
  val |= 1 << 0;
  write32((u32)&clk->rst_cfg, val);

  val = read32((u32)&clk->gate_cfg);
  val |= 1 << 0;
  write32((u32)&clk->gate_cfg, val);

  val = read32((u32)&clk->bus_cfg);
  val |= 1 << 0;
  write32((u32)&clk->bus_cfg, val);

  val = read32((u32)&clk->sel_cfg);
  val &= ~(1 << 0);
  write32((u32)&clk->sel_cfg, val);

  write32((u32)&glb->ctl, (1 << 0));
  write32((u32)&glb->status, 0);
  write32((u32)&glb->dbuff, 1);
  write32((u32)&glb->size, size);
  ...
  write32((u32)&bld->fcolor_ctl, 0x00000101);
  write32((u32)&bld->route, 1);
  write32((u32)&bld->premultiply, 0);
  write32((u32)&bld->bkcolor, 0xff000000);
  write32((u32)&bld->bld_mode[0], 0x03010301);
  write32((u32)&bld->bld_mode[1], 0x03010301);
  write32((u32)&bld->output_size, size);
  write32((u32)&bld->out_ctl, 0);
  write32((u32)&bld->ck_ctl, 0);
  for (i = 0; i < 4; i++) {
    write32((u32)&bld->attr[i].fcolor, 0xff000000);
    write32((u32)&bld->attr[i].insize, size);
  }

  write32(pdat->de + T113_DE_MUX_VSU, 0);
  write32(pdat->de + T113_DE_MUX_GSU1, 0);
  write32(pdat->de + T113_DE_MUX_GSU2, 0);
  write32(pdat->de + T113_DE_MUX_GSU3, 0);
  write32(pdat->de + T113_DE_MUX_FCE, 0);
  write32(pdat->de + T113_DE_MUX_BWS, 0);
  write32(pdat->de + T113_DE_MUX_LTI, 0);
  write32(pdat->de + T113_DE_MUX_PEAK, 0);
  write32(pdat->de + T113_DE_MUX_ASE, 0);
  write32(pdat->de + T113_DE_MUX_FCC, 0);
  write32(pdat->de + T113_DE_MUX_DCSC, 0);

  write32((u32)&ui->cfg[0].attr, (1 << 0) | (4 << 8) | (1 << 1) | (0xff << 24));
  write32((u32)&ui->cfg[0].size, size);
  write32((u32)&ui->cfg[0].coord, 0);
  write32((u32)&ui->cfg[0].pitch, 4 * pdat->width);
  write32((u32)&ui->cfg[0].top_laddr,
          (u32)(unsigned long)pdat->vram[pdat->index]);
  write32((u32)&ui->ovl_size, size);
}
...
static void t113_tconlcd_enable(t113_s3_lcd_t *pdat) {
  ...
  val = read32((u32)&tcon->gctrl);
  val |= (1 << 31);
  write32((u32)&tcon->gctrl, val);
}
...
static void t113_tconlcd_disable(t113_s3_lcd_t *pdat) {
  ...
  val = read32((u32)&tcon->dclk);
  val &= ~(0xf << 28);
  write32((u32)&tcon->dclk, val);

  write32((u32)&tcon->gctrl, 0);
  write32((u32)&tcon->gint0, 0);
}
...
static void t113_tconlcd_set_timing(t113_s3_lcd_t *pdat) {
  ...
  write32((u32)&tcon->timing1, ((total - 1) << 16) | ((bp - 1) << 0));
  bp = pdat->timing.v_sync_len + pdat->timing.v_back_porch;
  total = pdat->height + pdat->timing.v_front_porch + bp;
  write32((u32)&tcon->timing2, ((total * 2) << 16) | ((bp - 1) << 0));
  write32((u32)&tcon->timing3, ((pdat->timing.h_sync_len - 1) << 16) |
  ...
  write32((u32)&tcon->io_polarity, val);
  write32((u32)&tcon->io_tristate, 0);
}
...
static void t113_tconlcd_set_dither(t113_s3_lcd_t *pdat) {
  ...
  if ((pdat->bits_per_pixel == 16) || (pdat->bits_per_pixel == 18)) {
    write32((u32)&tcon->frm_seed[0], 0x11111111);
    write32((u32)&tcon->frm_seed[1], 0x11111111);
    write32((u32)&tcon->frm_seed[2], 0x11111111);
    write32((u32)&tcon->frm_seed[3], 0x11111111);
    write32((u32)&tcon->frm_seed[4], 0x11111111);
    write32((u32)&tcon->frm_seed[5], 0x11111111);
    write32((u32)&tcon->frm_table[0], 0x01010000);
    write32((u32)&tcon->frm_table[1], 0x15151111);
    write32((u32)&tcon->frm_table[2], 0x57575555);
    write32((u32)&tcon->frm_table[3], 0x7f7f7777);

    if (pdat->bits_per_pixel == 16)
      write32((u32)&tcon->frm_ctrl, (1 << 31) | (1 << 6) | (0 << 5) | (1 << 4));
    else if (pdat->bits_per_pixel == 18)
      write32((u32)&tcon->frm_ctrl, (1 << 31) | (0 << 6) | (0 << 5) | (0 << 4));
  }
}

static void fb_t113_cfg_gpios(int base, int n, int cfg, int pull, int drv) {
  for (; n > 0; n--, base++) {
    // todo
...
int lcd_init_sub(vga_device_t *vga, int mode) {
